...
def read_meta(in_dir, use_ndc):
    ...
    H, W, focal = c2ws[0, :, -1] # from the last column, we grab the height, width, and focal length

    print("Coming from COLMAP, the original poses_bounds (\"poses and bounds\") is: {}".format(poses_bounds))
    print("It appears that we explicitly extract...")
    print("Height: {}".format(H))
    print("Width: {}".format(W))
    print("Focal: {}".format(focal))
    print("Bounds: {}".format(bounds))

    print("Shape of images is {}".format(c2ws.shape))

    print("Now, the original camera-to-world-coordinates (c2ws) has rotation in \'down right back\':")
    print(c2ws[0,:,:-1])
    ...
    rotations_only = c2ws[:,:3,:3]
    print("Rotations only shape: {}".format(rotations_only.shape))
    rotations_as_quaternions = SO3_to_quat(rotations_only)
    print("Rotations as quaternions shape: {}".format(rotations_as_quaternions.shape))
    print("...and here are the quaternions:\n{}".format(rotations_as_quaternions))


    print("The new c2ws after converting to 'right up back\':")
    print(c2ws[0,:,:])

    # (N_images, 3, 4), (4, 4)
    c2ws, pose_avg = center_poses(c2ws)  # pose_avg @ c2ws -> centred c2ws

    print("Then, we \"center the poses\" and that same image now has pose data:")
    print(c2ws[0,:,:])
    ...
    if use_ndc:
        ...
        print("Supposing we're using NDC, minimum of the bounds gives near_original: {}".format(near_original))
        scale_factor = near_original * 0.75  # 0.75 is the default parameter
        # the nearest depth is at 1/0.75=1.33
        bounds /= scale_factor
        print("We scaled the bounds and now they are: {}".format(bounds))
        c2ws[..., 3] /= scale_factor
        print("As well, with the first image scaled the new scale gives us for c2ws:\n{}".format(c2ws[0,:,:]))
    

    c2ws = convert3x4_4x4(c2ws)  # (N, 4, 4)

    print("Finally, we get a 4x4 matrix version of the pose:")
    print(c2ws[0,:,:])
