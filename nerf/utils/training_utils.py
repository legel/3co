...
def heatmap_to_pseudo_color(heatmap, min_val=None, max_val=None):
    ...
    # normalize image so that relative depth is clearly highlighted

    if type(min_val) != type(None):
        minimum_depth_value = min_val
    else:
        minimum_depth_value = np.min(x)
    
    if type(max_val) != type(None):
        maximum_depth_value = max_val
    else:
...
class PolynomialDecayLearningRate():
    def __init__(self, optimizer, total_steps, start_value, end_value, exponential_index, curvature_shape, model_type, log_frequency):
        self._optimizer = optimizer
        self.total_steps = total_steps
        self.start_value = start_value
        self.end_value = end_value
        self.exponential_index = exponential_index
        self.curvature_shape = curvature_shape
    ...
    def polynomial_decay(self):
        return (self.start_value - self.end_value) * (1 - self.n_steps**self.curvature_shape / self.total_steps**self.curvature_shape)**self.exponential_index + self.end_value

    def _update_learning_rate(self):
        ''' Learning rate scheduling per step '''

        self.n_steps += 1                        
        lr = self.polynomial_decay()        
...
def gen_detail_name(args):
    outstr = str(datetime.datetime.now().strftime('%y%m%d_%H%M'))
    return outstr
...
# def set_randomness(random_seed=42):
