...
class PlaneFinder():
    def __init__(self, path_to_point_cloud_file, maximum_planes_to_fit = 1, minimum_points_per_plane = 10000):
        self.point_cloud_directory = path_to_point_cloud_file.split("/")[0:-1][0]
        ...
        self.maximum_point_to_plane_error_in_microns = 50
        self.maximum_number_of_trials = 1000
        self.point_to_plane_distance_threshold = 0.25 # maximum planarity error when adding new points, in millimeters
        self.outlier_to_remove_for_this_plane_threshold = self.point_to_plane_distance_threshold * 10
        self.minimum_total_planar_parameter_distance = 0.50 * 4
    ...
    def find_planes(self):
        point_cloud = self.read_point_cloud(self.point_cloud_filename)
        ...
        for trial_number in range(self.maximum_number_of_trials):
            ...
            parallel_cpu_computations = [self.add_points_in_plane.remote(sampled_points, points_to_process_for_cpu, best_planes) for points_to_process_for_cpu in points_to_process_for_cpus]
            ...
            if len(current_plane.points) > self.minimum_points_per_plane:   
                ...
                if error_per_point * 1000 < self.maximum_point_to_plane_error_in_microns: # ensure this plane is an excellent overall fit
                    is_unique = True
                    for best_plane in self.best_planes:
                        if not current_plane.unique_from([best_plane], self.minimum_total_planar_parameter_distance): # ensure this plane is sufficiently unique
                            print("  -> This plane is not unique, combining with one of our existing planes")
                            is_unique = False
                            self.best_plane.combine_planes(current_plane) # allow another plane to absorb this one
                            break
                    if is_unique:
        ...
        for combo in combinations( range(len(self.best_planes)), 2):
            ...
            for point_number, point in enumerate(plane_i.points):
                if plane_ii.get_planarity(point) < self.point_to_plane_distance_threshold * 7.5:
            ...
            for point_number, point in enumerate(plane_ii.points):
                if plane_i.get_planarity(point) < self.point_to_plane_distance_threshold * 7.5:
        ...
        print("Saving output plane point data to {}_plane_data.csv".format(self.point_cloud_identifier))
        with open("{}/{}_plane_data.csv".format(self.point_cloud_directory, self.point_cloud_identifier), "w") as output_file:
