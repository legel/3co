...
def draw_colorchecker(colors, centers, image, radius):
    for observed_color, expected_color, pt in zip(colors.reshape(-1, 3),
                                                  ...
                                                  centers.reshape(-1, 2)):
        x, y = pt
        cv.circle(image, (round(x), round(y)), radius//2, expected_color.tolist(), -1)
        cv.circle(image, (round(x), round(y)), radius//4, observed_color.tolist(), -1)
    return image
...
def write_results(colorchecker, filename=None):
    mes = 'color_index,pixel_x,pixel_y,r,g,b,real_r,real_g,real_b\n'
    real_rgb_colors = [(115, 82, 68), (194, 150, 130), (98, 122, 157), (87, 108, 67), (133, 128, 177), (103, 189, 170),
                        (214, 126, 44), (80, 91, 166), (193, 90, 99), (94, 60, 108), (157, 188, 64), (224, 163, 46),
                        (56, 61, 150), (70, 148, 73), (175, 54, 60), (231, 199, 31), (187, 86, 149), (8, 133, 161),
                        (243, 243, 242), (200, 200, 200), (160, 160, 160), (122, 122, 121), (85, 85, 85), (52, 52, 52)]
    mes_list = []
    print(colorchecker.values)

    i = 1
    for block in colorchecker.values:
        for r_g_b in block:
            (b, g, r) = (round(r_g_b[0]), round(r_g_b[1]), round(r_g_b[2]))
            mes_list.append((i, r, g, b))
            i += 1

    corners_box = calculate_edges(colorchecker)
    list_of_pixels = []

    n = 0
    for corners in corners_box:
        (real_r, real_b, real_g) = real_rgb_colors[n]
        (i, r, g, b) = mes_list[n]
        (x_min, x_max, y_min, y_max) = corners
        for y in range(int(y_min), int(y_max)):
            for x in range (int(x_min), int(x_max)):
                mes += '{},{},{},{},{},{},{},{},{}\n'.format(i, x, y, r, g, b, real_r, real_b, real_g)
        n += 1
...
if __name__ == '__main__':
    ...
    if len(argv) == 2:
        out, colorchecker = find_macbeth(argv[1])
        cv.imwrite(argv[1].replace(".png", "_detections.png"), out)
    # elif len(argv) == 4:
    #     out, colorchecker = find_macbeth(argv[1], patch_size=float(argv[3]))
    #     cv.imwrite(argv[2], out)
    else:
        print('Usage: %s <input_image>\n'
              '' % argv[0], file=stderr)
    write_results(colorchecker, argv[1].replace(".png", ".csv"))
