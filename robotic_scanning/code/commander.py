...
def move(directions, vertical_before_horizontal=True):
    ...
    camera_polarization = directions.get('camera_polarization', 'None')
    cali_x= directions.get('cali_x', 'None')
    cali_y= directions.get('cali_y', 'None')
    cali_z= directions.get('cali_z', 'None')
    cali_turn= directions.get('cali_turn', 'None')

    directions = {'x': x, 'y': y, 'z': z, 'phi': phi, 'theta': theta, 'turn': turn, 'projector_focus': projector_focus, 'camera_focus': camera_focus, 'camera_aperture': camera_aperture,'camera_polarization': camera_polarization, 'cali_x': cali_x,'cali_y': cali_y,'cali_z': cali_z,'cali_turn': cali_turn}
    for computer in computers:
        include_computer = False
        #print(computer)
        if computer in axis_to_computer.values():
            spots = {'x': 'None', 'y': 'None', 'z': 'None', 'phi': 'None', 'theta': 'None', 'turn': 'None', 'projector_focus': 'None', 'camera_focus': 'None', 'camera_aperture': 'None', 'camera_polarization': 'None', 'cali_x':'None','cali_y':'None','cali_z':'None','cali_turn':'None'}
            for axis in ['x', 'y', 'z', 'phi', 'theta', 'turn', 'projector_focus', 'camera_focus', 'camera_aperture', 'camera_polarization', 'cali_z', 'cali_x', 'cali_y', 'cali_turn']:
                computer_for_this_axis = axis_to_computer[axis]
                if computer_for_this_axis == computer:
                    #print(computer_for_this_axis)
                    spots[axis] = directions[axis]
                    if directions[axis] != 'None':
                        include_computer = True
            if include_computer:
                commands_for_computers[computer] = 'move?x={}&y={}&z={}&phi={}&theta={}&turn={}&projector_focus={}&camera_focus={}&camera_aperture={}&camera_polarization={}&cali_x={}&cali_y={}&cali_z={}&cali_turn={}'.format(spots['x'], spots['y'], spots['z'], spots['phi'], spots['theta'], spots['turn'], spots['projector_focus'], spots['camera_focus'], spots['camera_aperture'], spots['camera_polarization'], spots['cali_x'],spots['cali_y'],spots['cali_z'],spots['cali_turn'])
                #print(commands_for_computers)
...
def send(commands_for_computers, vertical_before_horizontal=True):
    if vertical_before_horizontal:
        computers_to_command = ['pi_2', 'pi_1', 'pi_3']
    else:
        computers_to_command = ['pi_1', 'pi_2', 'pi_3'] 

    # for computer in computers_to_command:
    #     commands = commands_for_computers[computer]

    for computer, commands in commands_for_computers.items():
        if type(commands) == type(''):
            commands = [commands]
        if len(commands) > 0:
            for command in commands:
                client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                #print("Trying to connect to {} at {}".format(computer, ip_addresses[computer]))
                client.connect((ip_addresses[computer], 8080))
                #print("Connected to {} at {}".format(computer, ip_addresses[computer]))
                client.send(command.encode())
                #print("Finished command to {} at {}".format(computer, ip_addresses[computer]))
                response_from_server = client.recv(4096)
                #print("Response from server: {}".format(response_from_server.decode()))
                client.close()
