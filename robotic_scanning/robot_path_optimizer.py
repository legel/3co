...
def generate_robot_poses(radius_count = 2, radius_min = 0.30, radius_max = 1.25, pitch_count = 2, pitch_min_value = -15, pitch_max_value = -85, yaw_count = 2, yaw_min_value = -30.0, yaw_max_value = 30.0):
	...
	for sweep_index, radius in enumerate(radius_range):
		...
		for pitch_index, pitch in enumerate(pitch_range):
			...
			for yaw in yaw_range:
				...
				x_offset_for_new_charuco_position_relative_to_block = 0.24
				x += x_offset_for_new_charuco_position_relative_to_block

				# x = np.random.normal(loc=x, scale=robot_x_standard_deviation)
				# y = np.random.normal(loc=y, scale=robot_y_standard_deviation)
				# z = np.random.normal(loc=z, scale=robot_z_standard_deviation)
				# pitch = np.random.normal(loc=pitch, scale=robot_pitch_standard_deviation)
				# yaw = np.random.normal(loc=yaw, scale=robot_yaw_standard_deviation)
...
def plan_path_to_minimize_distance_traveled(robot_poses):
	start_time = time.time()

	number_of_poses_to_sort = robot_poses.shape[0]
	number_of_dimensions_per_pose = robot_poses.shape[1]
